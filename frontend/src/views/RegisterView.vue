  ...
  <div class="auth-page">
    <div ref="authCardRef" class="clean-card auth-card">
      ...
      <div class="auth-header">
        <div class="brand-box">
          <UserPlus :size="28" stroke-width="2.2" />
        </div>
        <h2 class="auth-title">Create Account</h2>
        <p class="auth-desc">Register a new system administrator for EdgeGuard</p>
      </div>
      ...
      <form @submit.prevent="handleRegister" class="auth-form">
        ...
        <button type="submit" class="btn btn-primary auth-submit-btn" :disabled="auth.loading">
          <UserCheck :size="16" />
          <span>{{ auth.loading ? 'Creating...' : 'Register Account' }}</span>
        </button>
      </form>
...
import { UserPlus, AlertCircle, UserCheck } from 'lucide-vue-next';
